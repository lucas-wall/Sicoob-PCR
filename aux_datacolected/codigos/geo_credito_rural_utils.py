...
def plot(imagem_cortada: np.ndarray, bbox: tuple, classes: gpd.GeoDataFrame):
    ...
    for value in values:
        ...
        if value == -9999:
            ...
            class_info = classes[classes['Value'] == value]
            if not class_info.empty:
                cmap_colors.append(class_info['Color'].values[0])
                height_class_labels.append(class_info['Category'].values[0])
            else:
    ...
    total_pixels = np.sum(num_occurrences[values != -9999])
    areas = (num_occurrences[values != -9999] * 30 * 30) / 10000
    percentages = (num_occurrences[values != -9999] / total_pixels) * 100

    # Organizar informações de classe em um DataFrame
    class_info = classes.set_index('Value').loc[values[values != -9999]]
    class_info['Area (ha)'] = areas
    class_info['Percentage (%)'] = percentages
    class_info = class_info[['Label', 'Category', 'Area (ha)', 'Percentage (%)']]
